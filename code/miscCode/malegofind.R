...
termList <- read.delim('goPost/termFinalM', sep='\n', header=FALSE)
...
geneFinal <- read.delim('goPost/geneLabelsM', sep='\n', header = FALSE)

finalData <- cbind(filt1, geneFinal)
colnames(finalData) <- c('flybase','count','gene')


saveRDS(finalData, 'goPost/finalDataM.Rds')
